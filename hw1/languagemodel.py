...
# test function to set up iPython
def sayHello():
    print('hello')
    ...
    for snt in corpus:
        for w in snt:
            if (not tmpindex.has_key(w)):
                tmpindex[w] = indx
                indx += 1
    # indx number of unique words
    # tmpindex has unique index for unique words
...
def ngramGen(corpus, w2index, n):
    ...
    return ngrams
# list of pairs that occured in the corpus. There can be duplicate entries
# The number of entires correspond to the frequencies
# for example, [[1,2], [1,2]] means word_1 and word_2 bigram occurred twice
...
def bigramLM(corpus, w2index, nwords, alpha=0.0):
    ...
    return prob
# rowindex is the index of the conditional word
# colindex is the index of the target word
