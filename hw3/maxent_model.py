...
if modelnumber == 1:
    feature_list = np.array( [f for fname, f in sorted(globals().items()) if callable(f) and fname[0] == 'z' and (len(fname) == 2 or len(fname)==3)] )
...
words = [ r.rsplit('_', 1)[0] for r in raw ]
tags = [ r.rsplit('_', 1)[0] for r in raw ]
...
if modelnumber == 2:
    ...
    predict output
    """
    input_dev = open(testdata, 'rb')
    output = open('./output_test%d.tag'%modelnumber, 'w')
    sys.stdout = output
