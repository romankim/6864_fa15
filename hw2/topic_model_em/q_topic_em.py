...
def e_step(fileData, theta_t_z, theta_z_w):
    ...
    for t in range(NUM_DOCS):
        ...
        ### in the document
        for w in fileData[t]:
            posterior_over_z = theta_t_z[t] * theta_z_w[:,w]            
            posterior_over_z = posterior_over_z / np.sum(posterior_over_z)
            posterior_w_z[w] = posterior_over_z
            ...
            zcount = np.zeros(NUM_TOPICS)
            for k,v in posterior_w_z.iteritems():
                zcount = zcount + v
                
            count_t_z[t] = zcount
            count_w_z[w] = count_w_z[w] + posterior_w_z[w]
...
def m_step(count_t_z, count_w_z):
    '''TODO: Get the max Likelihood estimate of theta_t_z[t][z]
         Make sure that the theta_t_z[t] is a valid probability 
                    distribution over the topics z.'''
    normalsum = count_t_z.sum(axis=1)
    theta_t_z = count_t_z / normalsum[:, np.newaxis]
    ...
    normalsum = count_w_z.sum(axis=1)
    theta_z_w = (count_w_z / normalsum[:, np.newaxis]).T
...
if __name__ == '__main__':
    input_directory = r'C:\Users\Dongyoung\Desktop\Lab\classes\!6.864\hw2\topic_model_em\data\nyt'
